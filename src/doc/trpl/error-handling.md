...
scenarios that are
[beyond the scope of this chapter](https://crates.io/crates/error).)
...
This chapter was long, and depending on your background, it might be rather
...
Let's get argument parsing out of the way. we won't go into too much detail on
...
coding a problem, error handling is usually the last thing we want to think
...
personally fond of it because we like using `enum`s for representing errors, but
